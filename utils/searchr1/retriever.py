...
def search(query: str, topk: int = 3, return_scores: bool = True):
    """
    ...
    if return_scores:
        return {"passages": _passages2string(results[0]), "scores": [[doc['score'] for doc in results[0]]]}
    return {"passages": _passages2string([results[0]])}